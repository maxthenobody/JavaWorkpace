...
public class B_Switch {
	 ...
	 * switch 문
	 *  - if 문과 동일한 조건문
	 *  - if 문은 조건식을 복잡하게 기술할 수 있음. 값의 범위를 제시.
	 *    하지만, switch 문은 확실한 하나의 값만 기술 (동등비교만 수행가능).
	 *    
	 * [표현법]
	 * switch (동등비교할 계산식, 변수) {
	 *     case 값1 : 실행할 코드; // switch 문의 변수와 값1이 동등한 경우 실행하는 코드.
	 *     			  break; // 코드 실행 후 switch 문을 탈출하는 역할 연산자.
	 ...
	 * }
	 * 
	 * switch 문의 장점 -> case 를 통해 내가 실행하기를 원하는 코드로 즉시 이동하기 때문에 연산속도가 if ~ else if 보다 빠르다.
	 * 
	 * switch 문의 사용처
	 *  - 항상 동일한 속도로 연산이 수행되어야 하는 경우
	 *    ex) 키보드 입력, 마우스 입력
	 *  - 가독성측면에서 if ~ else if 문보다 더 뛰어난 경우
	 *    ex) 동등비교 조건식을 5회 이상 사용하는 경우 switch 문 사용 권장
	 */
