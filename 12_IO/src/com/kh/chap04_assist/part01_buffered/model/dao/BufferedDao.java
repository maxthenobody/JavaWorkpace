...
public class BufferedDao {
	 ...
	 * -> 데이터를 바로 출력/입력 하는 것이 아니라 크기가 큰 버퍼공간을 만들어서 데이터를 모아두었다가 한번에 입출력을 진행
	 * 
	 */
...
//		BufferedWriter bw = null;
		...
		// 만약 보조스트림이 Writer 면 기반스트림도 Writer 여야 함.
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("c_buffer.txt"))) {
			...
			// flush() 메서드는 close() 메서드 실행시 자동으로 호출되는 메서드
			// 버퍼공간이 다 채워지지 않은 상태에서 메서드가 종료될때, 버퍼에 담겨있는 데이터가 누락될 수 있으므로
			// flush 함수를 통해 버퍼를 비워줘야 한다.
