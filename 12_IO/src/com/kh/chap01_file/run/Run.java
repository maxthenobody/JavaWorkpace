...
public class Run {
	public static void main(String[] args) {
		...
		// - 파일의 속성, 경로, 이름, 그 밖의 다양한 데이터등을 얻을 수 있다.
		...
		try {
			...
			File bbbFolder = new File("C:/bbb"); // 확인하고자 하는 경로를 가진 File 객체 생성
			boolean doesExist = bbbFolder.exists();
		 ...
		 * 이 통로를 Stream 이라고 부른다.
		 * 
		 * 스트림의 특징
		 * - 단방향: 입력이면 입력, 출력이면 출력으로만 데이터 전송이 가능.
		 * - 선입선출(First In First Out): 먼저 전달한 값이 먼저 나오게 된다.
		 * - 시간지연문제가 발생할 수도 있다.
		 * 
		 * 스트림의 구분
		 * - 데이터의 크기에 따른 분류
		 *   1) 바이트 스트림(1 byte) (InputStream/OutputStream)
		 *   - 모든 바이트 스트림은 InputStream/OutputStream 추상 클래스를 상속받아 구현.
		 *   - 읽기는 read(), 쓰기는 write() 를 사용.
		 *   - 1 byte 짜리 데이터가 오고갈 수 있는 스트림.
		 *   
		 *   2) 문자 스트림(2 byte) (Reader/Writer)
		 *   - 모든 문자 스트림은 Reader/Writer 추상 클래스를 상속받아 구현했다.
		 *   - 읽기는 read(), 쓰기는 write() 함수를 사용.
		 *   - 2 byte 단위로 데이터가 오고가는 스트림.
		 *   
		 * - 외부매체와의 연결 가능 여부
		 *    기반 스트림: 외부매체와 직접연결이 가능한 스트림.
		 *    보조 스트림: 기반 스트림의 부족한 기능을 보조하기 위한 스트림.
		 *              (속도 향상, 자료형 반환, 객체단위 입출력...)
		 *              단독으로 사용이 불가능하며, 기반 스트림이 필수.
		 * 
		 */
