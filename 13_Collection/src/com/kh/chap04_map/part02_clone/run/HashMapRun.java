package com.kh.chap04_map.part02_clone.run;
...
//		}
	...
	public int getIndex(K key) {
		// 매개변수로 전달받은 key 를 활용하여 table 의 저장할 인덱스 위치를 구하는 함수.
		// key 값에 hashCode() 를 적용
		// hashCode 값이 음수일 수 있으므로, 절대값으로 변환
		// 배열의 크기만큼 나머지 연산 수행
		// 113355 % 10 => 5번 인덱스
		// 12345678 % 10 => 8번 인덱스
		// 77777788 % 10 => 8번 인덱스
	...
	public void put(K key, V value) {
		...
		// table 의 index 위치에 값이 초기화 되어있는지 확인하기.
		if (table[index] == null) {
			table[index] = newEntry;
		} else {
			// index 위치에 이미 초기화된 데이터가 들어가 있다면?
			// "다이제", "다이제" => 중복 키값
			//  - 덮어쓰기
			if (table[index].key.equals(newEntry.key)) {
				table[index].value = newEntry.value;
				return; // size 가 추가되지 않도록 종료.
			} else {
				// "다이제", "칸초" => 우연히 hashCode 중복으로 충돌이 발생한 경우.
				//  - 다이제가 저장된 entry 의 next 에 칸초를 저장.
	...
	public void remove(K key) {
		...
		do {
			if (e.key.equals(key)) {
				// 첫번째 entry 인지 확인			
				if (e == table[index]) {
					// e.next 값이 null 이 아니라면 e.next 값을 대입.
