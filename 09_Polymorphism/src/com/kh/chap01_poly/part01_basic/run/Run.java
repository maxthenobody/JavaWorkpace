...
public class Run {
	public static void main(String[] args) {
		...
		System.out.println("2. 자식타입 참조변수로 자식객체 다루기.");
		...
		Parent p3 = /*(Parent)*/ new Child2(); // 자동형변환(업캐스팅)
		// 자식에서 부모클래스로 자동형변환 ==> 업캐스팅
		// 상속 관계에서만 적용된다.
		
		((Child1)p2).printChild1(); // 다운캐스팅 (부모에서 자식으로)
		// p2 참조변수로는 Parent 에 있는 메서드에만 접근이 가능하므로, Child1 클래스로 강제형변환
		// 시켜 주어야, Child1 내부의 메서드에 접근 가능하다.
		 ...
		 *  - 여러개의 자식타입 중 하나를 직접 기술 해줘야하므로 개발자가 형변환 연산자를 명시적으로 작성해 줘야한다.
		 */
...
//		System.out.println(p1);
		...
		arr[3] = new Child1(10, 11, 12);
		
		System.out.println("==================");
		
		for (int i = 0; i < arr.length; i++) {
			// 각 인덱스에 초기화된 자식객체 내부의 printChild 메서드 호출하기.
