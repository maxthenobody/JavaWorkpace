...
//		run.test1();
	...
	public void test2() {
		 ...
		 * 2) interrupt(): sleep()이나, join()에 의해 일시정지된 스레드를 실행대기(깨우는)상태로 만든다. (에러 강제 발생) 
		 * 
		 */
	...
	public void test1() {
		 ...
		 *  2) wait(): 동기화된 블럭 안에서만 사용되며, 지정된 시간이 지나거나, 다른 스레드가 호출하기 전까지 대기시키는 메서드.
		 *  3) join(): 지정된 시간동안 다른 스레드를 실행시키고, 현재 스레드는 대기시키는 메서드.
		 *             지정된 시간이 지나거나, 다른 스레드의 작업이 완료되면 현재 스레드가 다시 가동된다.
		 *  4) IO Block(입출력메서드)
		 *  
		 */
		// 0.05초간 일시정지하는 스레드.
