...
public class B_StringBuilderAndBuffer {
	 ...
	 *  - 문자열 기반 가변클래스(불변의 반대: 수정 가능한지 아닌지 여부)
	 *  - 문자열의 메모리 부족 이슈를 해결하기 위해 만들어진 클래스
	 *  - 두 클래스는 동일한 메서드를 가지고 있다.
	 *  - 차이점은 "동기화 여부" 하나.
	 *  - StringBuilder 는 동기화를 지원하지 않기 때문에 "멀티스레드 환경"에서도 좋은 성능을 보인다. 99% 빌더 씀.
	 *  - StringBuffer 는 동기화를 지원하기 때문에 "멀티스레드 환경"에서 스레드 안정성을 보장할 수 있다. 단 성능이 좋지 못하다. 동기화를 쓰면 느려짐.
	 * 
	 */
	...
	public void method() {
		...
		System.out.println(sb2.indexOf("W")); // 처음 나오는 W
